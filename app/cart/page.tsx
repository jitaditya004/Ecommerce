...
export default function CartPage() {
  ...
  >({
    ...
    onMutate: async ({ id, delta }) => {
      ...
      const changedItem = previousCart?.items.find((item) => item.id === id);

      if (changedItem) {
        setStockErrors((prev) =>
          prev.filter(
            (error) => error.productId !== changedItem.products.product_id,
          ),
        );
      }
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-zinc-400">₹ {item.products.price}</p>

                  {error && (
                    <p className="text-red-400 text-sm mt-1">
                      Only {error.available} left. You selected{" "}
                      {error.requested}.
                    </p>
                  )}
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    {item.quantity >= item.products.stock && (
                      <p className="text-red-500 text-sm">Max stock reached</p>
                    )}

                    <button
                      type="button"
                      onClick={() =>
                        updateQtyMutation.mutate({ id: item.id, delta: -1 })
                      }
                    ...
                    <button
                      type="button"
                      disabled={
                        item.quantity >= item.products.stock || isMutating
                      }
                      onClick={() =>
                        updateQtyMutation.mutate({ id: item.id, delta: 1 })
                      }
